...
namespace Clingo
{
    ...
    {
        private const string clingoServerURL = "https://clingo.onrender.com/clingo";
        ...
        IEnumerator Upload()
        {
            ...
            using (UnityWebRequest req = UnityWebRequest.Post(clingoServerURL, form))
            {
                ...
                else
                {
                    ...
                    else if (answerSet.Result == "UNSATISFIABLE")
                    {
                        status = Status.UNINITIATED;
                    }
                    else if (answerSet.Result == "TIMEOUT")
                    {
                        status = Status.TIMEDOUT;
                    }
                    else if (answerSet.Result == "ERROR")
                    {
                        status = Status.ERROR;
                    }
                }
